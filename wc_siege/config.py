...
# "schedule"  -> FBref's fixtures table, one row per GAME, carrying xG for both sides.
#                The opponent's xG is the shot QUALITY a keeper's side absorbed (xG-faced),
#                which is one of the dimensions the Reddit critics named for Spain 0-0
#                Cape Verde (2.1 xG vs 0.2). It is also a single page per tournament, so it
#                is fast and rarely blocked, unlike the per-squad shooting pull.
#
# IMPORTANT LIMITATION: soccerdata's read_team_match_stats only supports
# ['schedule', 'shooting', 'keeper', 'misc'] -- there is NO possession table. So possession %
# and touches-in-the-box (the most literal "pinned in its own box" measures) are NOT
# obtainable through this pipeline. Documented in the README rather than faked.
STAT_TYPES = ["keeper", "shooting", "schedule"]

# Columns are matched fuzzily in clean.py (exact flattened names, e.g. "Standard_Sh", are
# confirmed against the real pull) so we survive FBref's header grouping.
